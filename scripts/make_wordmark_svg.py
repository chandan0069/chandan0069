"""
Render "Chandan" as an EXTRUDED 3D wordmark rasterized to ASCII, and emit it as an
...
TEXT = os.environ.get("WORDMARK_TEXT", "CHANDAN")

MASK_H = 360           # higher raster density preserves small counters and diagonals
TRACKING = 0.18        # modest spacing keeps CHANDAN legible without separating letters too much. counter gaps must survive the
...
DEPTH_FRAC = 0.24      # slightly shallower extrusion keeps the face readable at ASCII resolution
...
LIGHT = np.array([-0.10, -0.25, -1.00])
LIGHT = LIGHT / np.linalg.norm(LIGHT)
AMBIENT = 0.12
FOG = 0.18             # how much the far end of the word dims, 0..1
...
def build_shell():
    ...
    # Expand strokes slightly before voxelization so thin ASCII strokes stay connected.
...
def emit(frames, mode, out, dur, reveal):
    ...
    p.append(f'<text x="{canvas_w/2:.0f}" y="{TITLEBAR_H/2 + 4:.0f}" fill="{TITLE_TEXT}" '
             f'font-size="11.5" text-anchor="middle">chandan0069@github: ~$ ./wordmark.sh --3d</text>')
